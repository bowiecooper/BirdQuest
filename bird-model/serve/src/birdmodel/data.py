...
class NABirds(Dataset):
    """NABirds dataset split. Set ``train=True`` for training, else test.

    NABirds has ~1011 classes in its hierarchy, but only 555 are actually used
    in the dataset. We remap used class IDs to 0-indexed sequential labels.
    """
    ...
    def __init__(self, root: str | Path, train: bool, transform=None):
        ...
        if not (self.root / "images").exists():
            raise FileNotFoundError(
                f"NABirds images not found under {self.root}."
            )
        self.transform = transform

        paths = self._read_pairs("images.txt")
        raw_labels = {i: int(c) for i, c in self._read_pairs("image_class_labels.txt").items()}
        is_train = {i: c == "1" for i, c in self._read_pairs("train_test_split.txt").items()}
        raw_classes = self._read_pairs("classes.txt")

        # Collect all used class IDs and build a mapping to 0-indexed sequential IDs
        used_class_ids = sorted(set(raw_labels.values()))
        self.class_id_to_idx = {cid: idx for idx, cid in enumerate(used_class_ids)}

        # Remap labels to sequential 0-indexed
        labels = {i: self.class_id_to_idx[c] for i, c in raw_labels.items()}
        ...
        # Build class names in the same order (by remapped index)
        self.classes = [
            pretty_class_name(raw_classes[str(cid)]) for cid in used_class_ids
        ]
...
def build_dataloaders(
    ...
    Automatically detects CUB-200 vs NABirds dataset based on directory structure.
    """
    train_tf, eval_tf = build_transforms(img_size)
    root = Path(root)

    # Detect dataset type: NABirds has hierarchy.txt, CUB-200 doesn't
    is_nabirds = (root / "hierarchy.txt").exists()

    if is_nabirds:
        train_ds = NABirds(root, train=True, transform=train_tf)
        test_ds = NABirds(root, train=False, transform=eval_tf)
    else:
